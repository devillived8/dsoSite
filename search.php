...
function selectAllAcounts()
{
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "
                            <a target='_blank' href='character.php?idCharacter=". urlencode($row['idCharacter']). "&class=" . urlencode($row['class']) . "&price=" . urlencode($row['price']) . "&date=" . urlencode($row['date']) . "&img=" . urlencode($row['img']) . "&description=" . urlencode($row['description']) . "&server=". urlencode($row['server']) . "&wayToGet=" . urlencode($row['wayToGet']) ."' class='linkOnCharacter' data-date='{$row['date']}' data-price='{$row['price']}'>
                            <div class='character'>
...
function selectAccountByNickname()
{
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "
            <a target='_blank' href='character.php?idCharacter=". urlencode($row['idCharacter'])."&description=" . urlencode($row['description']) . "&class=" . urlencode($row['class']) . "&price=" . urlencode($row['price']) . "&date=" . urlencode($row['date']) . "&img=" . urlencode($row['img']) . "&server=". urlencode($row['server']) . "&wayToGet=" . urlencode($row['wayToGet']) . "' class='linkOnCharacter' data-date='{$row['date']}' data-price='{$row['price']}'>
                <div class='character'>
                    <div class='character__wrapperClass'>
